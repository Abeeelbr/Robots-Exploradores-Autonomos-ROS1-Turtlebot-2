...
class InterfazManager:
    ...
    def map_callback(self, msg):
        ...
        try:
            ...
            scale_width, scale_height = 700, 700  # Dimensiones visualizadas
            ...
            for station_name, (x, y) in almacen_estaciones.items():
                pixel_x = int((x - msg.info.origin.position.x) / msg.info.resolution)
                pixel_y = int((y - msg.info.origin.position.y) / msg.info.resolution)

                # Ajustar coordenadas invertidas y escala
                pixel_x = int(pixel_x * scale_x)
                pixel_y = int((height - pixel_y) * scale_y)

                radius = 5
                draw.ellipse((pixel_x - radius, pixel_y - radius, pixel_x + radius, pixel_y + radius), fill="red")
                draw.text((pixel_x + radius, pixel_y - radius), station_name, fill="black", font=font)

            # Obtener la pose del robot
            robot_pose = self.get_robot_pose()
            if robot_pose:
                robot_x, robot_y, robot_theta = robot_pose
                pixel_x = int((robot_x - msg.info.origin.position.x) / msg.info.resolution * scale_x)
                pixel_y = int((height - (robot_y - msg.info.origin.position.y) / msg.info.resolution) * scale_y)
