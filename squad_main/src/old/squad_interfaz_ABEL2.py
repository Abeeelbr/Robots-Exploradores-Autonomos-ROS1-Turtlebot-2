...
from squad_exploracion.msg import (
    ObjectDetectionAction, ObjectDetectionGoal, ObjectDetectionFeedback,
    AutonomousControlAction, AutonomousControlGoal, AutonomousControlResult,
    ApproachControlAction, ApproachControlFeedback, ApproachControlResult, ApproachControlGoal,
    DetectedObject
)
...
class InterfazManager:
    ...
    def __init__(self):
        ...
        self.map_sub = rospy.Subscriber(self.map_topic, ROSImage, self.map_callback)
    ...
    def map_callback(self, msg):
        ...
        Convierte la imagen del mapa recibida de ROS a un formato compatible con Tkinter y la
        muestra en la GUI.

        Args:
            msg (ROSImage): Mensaje de imagen recibido del tópico del mapa.
        """
        try:
            cv_map = self.bridge.imgmsg_to_cv2(msg, desired_encoding='bgr8')
            # Convertir la imagen de BGR a RGB
            cv_map = cv2.cvtColor(cv_map, cv2.COLOR_BGR2RGB)
            # Convertir la imagen a PIL
            pil_map = Image.fromarray(cv_map)
            # Redimensionar la imagen para ajustarla al tamaño del label
            pil_map = pil_map.resize((700, 700), Image.LANCZOS)  # Tamaño ajustado al nuevo ancho del marco
            # Convertir la imagen a ImageTk
            ...
            rospy.logerr(f"Error al convertir la imagen del mapa: {e}")
        except Exception as e:
            rospy.logerr(f"Error inesperado en map_callback: {e}")
