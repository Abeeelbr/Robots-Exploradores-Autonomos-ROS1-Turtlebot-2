...
class InterfazManager:
    ...
    def setup_gui(self):
        ...
        self.window.geometry("1400x800")  # Tamaño ajustable según necesidad
        ...
        self.left_frame = tk.Frame(self.bottom_frame, bg='#d0d0d0', width=700)
        ...
        self.camera_frame.pack(side=tk.TOP, fill=tk.BOTH, expand=True, padx=10, pady=10)
    ...
    def map_callback(self, msg):
        ...
        Convierte el mensaje OccupancyGrid a una imagen de PIL y la muestra en la GUI.
        Además, sobrepone las estaciones y la posición del robot como círculos con etiquetas.

        Args:
            msg (OccupancyGrid): Mensaje de mapa recibido del tópico '/map'.
        """
        try:
            # Convertir el OccupancyGrid a una imagen de 8 bits
            width = msg.info.width
            height = msg.info.height
            data = msg.data

            # Crear una imagen en escala de grises (0-255)
            # Asignar 0 a los píxeles ocupados, 255 a los libres, y 128 a los desconocidos
            grayscale = []
            for value in data:
                if value == 0:
            ...
            # Convertir la lista a una imagen de PIL
            pil_map = Image.new('L', (width, height))
            pil_map.putdata(grayscale)
            pil_map = pil_map.convert('RGB')  # Convertir a RGB para dibujar en color

            # Extraer parámetros del mapa
            self.map_resolution = msg.info.resolution  # metros por píxel
            self.map_origin_x = msg.info.origin.position.x  # Coordenada X del origen
            self.map_origin_y = msg.info.origin.position.y  # Coordenada Y del origen

            # Redimensionar la imagen para ajustarla al tamaño del label
            pil_map = pil_map.resize((700, 700), Image.NEAREST)  # Tamaño ajustado al nuevo ancho del marco

            # Crear un objeto de dibujo
            draw = ImageDraw.Draw(pil_map)
            font = ImageFont.load_default()

            # Dibujar estaciones sobre el mapa
            for station_name, (x, y) in almacen_estaciones.items():
                # Transformar coordenadas del mundo real a píxeles
                pixel_x = int((x - self.map_origin_x) / self.map_resolution)
                pixel_y = int((y - self.map_origin_y) / self.map_resolution)

                # Ajustar para la inversión del eje Y en la imagen
                pixel_y = height - pixel_y

                # Escalar a la nueva imagen del mapa
                scale_x = pil_map.width / width
                scale_y = pil_map.height / height
                pixel_x = int(pixel_x * scale_x)
                pixel_y = int(pixel_y * scale_y)

                # Dibujar un círculo para la estación
                radius = 5
                draw.ellipse(
                ...
                # Dibujar el nombre de la estación
                draw.text(
            ...
            if robot_pose:
                ...
                # Transformar coordenadas del mundo real a píxeles
                pixel_x = int((robot_x - self.map_origin_x) / self.map_resolution)
                pixel_y = int((robot_y - self.map_origin_y) / self.map_resolution)
                ...
                # Dibujar un triángulo para representar al robot
                size = 10  # Tamaño del triángulo
                angle = robot_theta  # Ángulo del triángulo basado en la orientación del robot

                # Calcular los puntos del triángulo
                point1 = (
                    pixel_x + size * math.cos(angle),
                    pixel_y + size * math.sin(angle)
                )
                point2 = (
                    pixel_x + size * math.cos(angle + 2.5),
                    pixel_y + size * math.sin(angle + 2.5)
                )
                point3 = (
                    pixel_x + size * math.cos(angle - 2.5),
                    pixel_y + size * math.sin(angle - 2.5)
                ...
                # Opcional: Dibujar una línea indicando la dirección
                draw.line([pixel_x, pixel_y, point1[0], point1[1]], fill='black', width=2)

                # Opcional: Dibujar el nombre del robot
                draw.text(
