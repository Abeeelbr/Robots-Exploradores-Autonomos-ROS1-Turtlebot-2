        ...
        while not rospy.is_shutdown():
            ...
            if self.objeto_detectado:
                if self.acercarse_objetos:
                    ...
                    rospy.loginfo("Objeto detectado, volviendo a HOME...")
...
class InterfazManager:
    ...
    def __init__(self, state_machine):
        ...
        # Inicializar variables para el control de la cámara
        self.last_image_time = rospy.Time.now()
        self.image_timeout = rospy.Duration(1.0)  # Tiempo de espera de 1 segundo
        ...
        # Iniciar el chequeo periódico del tópico de la cámara
        self.check_camera_topic()
    ...
    def update_state_highlight(self):
        ...
        # Ya no cambiamos el tópico de la cámara aquí; se gestiona en check_camera_topic
    ...
    def subscribe_to_camera(self, topic):
        ...
        rospy.loginfo("Suscrito al tópico de cámara: %s", topic)

        # Al cambiar de tópico, reiniciar el tiempo de la última imagen
        self.last_image_time = rospy.Time.now()
    ...
    def camera_callback(self, msg):
        ...
        try:
            ...
            # Actualizar el tiempo de la última imagen recibida
            self.last_image_time = rospy.Time.now()
    ...
    def check_camera_topic(self):
        """
        Verifica si se están recibiendo imágenes del tópico de la cámara procesada.

        Si no se reciben imágenes en un tiempo determinado, cambia al tópico de la cámara normal.
        """
        now = rospy.Time.now()
        time_since_last_image = now - self.last_image_time

        if self.current_camera_topic == self.action_camera_topic:
            if time_since_last_image > self.image_timeout:
                # No se reciben imágenes del tópico procesado, cambiar al normal
                rospy.logwarn("No se reciben imágenes del tópico %s. Cambiando al tópico normal.", self.current_camera_topic)
                self.switch_camera_topic(self.normal_camera_topic)
        else:
            # Intentar cambiar al tópico procesado
            self.switch_camera_topic(self.action_camera_topic)
            rospy.loginfo("Intentando cambiar al tópico de la cámara procesada.")

        # Programar la siguiente verificación
        self.window.after(500, self.check_camera_topic)  # Verificar cada 500 ms

    # ************************************************
    #            GESTIÓN CIERRE PROGRAMA
    # ************************************************
