...
class TurtleBotObjectDetectionAction:
    ...
    def __init__(self):
        ...
        # Cargar el modelo YOLO en el dispositivo especificado
        try:
            self.model = YOLO('yolov8n.pt')
            self.model.to(self.device)
            rospy.loginfo("Modelo YOLOv8 cargado exitosamente en GPU." if self.device == 'cuda' else "Modelo YOLOv8 cargado exitosamente en CPU.")
    ...
    def callback(self, image_msg, depth_msg):
        ...
        try:
            with self.lock:
                ...
                # Obtener la pose del robot usando TF
                self.latest_position = self.get_robot_pose()
    ...
    def execute(self, goal):
        ...
        rospy.loginfo(f"Frecuencia de procesamiento: {self.process_frequency} Hz")
        while not rospy.is_shutdown():
            ...
            # rospy.loginfo('Llamando a process_latest_data()...')
    ...
    def process_latest_data(self, feedback):
        ...
        """
        rospy.loginfo('Procesando datos más recientes...')
        with self.lock:
            # Verificar que todos los datos necesarios estén disponibles
            # rospy.loginfo(f'Datos necesarios: {self.latest_image } {self.latest_position } {self.latest_depth_cv } {self.fx } {self.fy } {self.cx } {self.cy}')
            if (self.latest_image is not None and 
                ...
                rospy.loginfo('Datos validaddos | Procesando imagen...')
                try:
                    ...
                    rospy.loginfo(f"Objetos detectados: {len(detected_objects)}")
                    ...
                    # rospy.loginfo(f"Objetos en coordenadas mundiales: {len(detected_objects_world)}")
    ...
    def detect_objects(self, cv_image):
        ...
        Returns:
            tuple: Imagen procesada con anotaciones y lista de objetos detectados con sus coordenadas de píxel.
        """
        detected_objects = []

        try:
            # Realizar la inferencia con YOLOv8 en el dispositivo especificado
            results = self.model(cv_image, device=self.device, classes=self.classes_of_interest)
            ...
            for detection in detections.boxes:
                ...
                # Filtrar detecciones con confianza mayor al 70%
                if conf < 0.7:
                    continue
            ...
            rospy.logerr(f"Error en la detección de objetos con YOLOv8: {e}")
