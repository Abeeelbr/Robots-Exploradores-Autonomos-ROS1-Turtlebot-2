...
de objetos utilizando YOLOv8, el cálculo de coordenadas mundiales de los objetos detectados y la publicación
...
class TurtleBotObjectDetectionAction:
    ...
    def __init__(self):
        ...
        # ************************************************
        try:
            # Asegúrate de que la ruta al modelo YOLOv8 es correcta
            self.model = YOLO('/home/abel/robots_moviles_ws/src/robots_exploradores/squad_exploracion/src/yolov8n.pt')
            rospy.loginfo("Modelo YOLOv8 cargado exitosamente.")
        except Exception as e:
            rospy.logerr(f"Error al cargar el modelo YOLOv8: {e}")
            return

        # Clases de interés (ID de las clases)
        self.classes_of_interest = [0,1,2,3,7,10,13,24,32,56,57,59,60,61,62,70,72]
        ...
        if not self.done_calibrations:
            try:
                with self.lock:
                    ...
                    self.done_calibrations = True

                rospy.loginfo("Datos iniciales recibidos y calibraciones completadas.")
    ...
    def camera_info_callback(self, msg):
        ...
        with self.lock:
            ...
            self.cx = msg.K[2]
            self.cy = msg.K[5]
            # Asegúrate de que msg.header.frame_id no comience con '/'
    ...
    def process_latest_data(self, feedback):
        ...
        with self.lock:
            ...
            if (self.latest_image is not None and 
                ...
                rospy.loginfo("Todos los datos necesarios están disponibles para el procesamiento.")
                try:
                ...
                rospy.logwarn("Faltan datos para procesar. Esperando a recibir todos los datos necesarios.")
    ...
    def detect_objects(self, cv_image):
        """
        Detecta objetos utilizando YOLOv8 en una imagen RGB.

        Args:
            cv_image (ndarray): Imagen RGB en formato OpenCV.

        Returns:
            tuple: Imagen procesada y lista de objetos detectados con sus coordenadas y etiquetas.
        """
        detected_objects = []

        try:
            # Realiza la inferencia con YOLOv8 y filtra las clases de interés
            results = self.model(cv_image, classes=self.classes_of_interest)
            # Acceder a la primera imagen de los resultados
            ...
            for detection in detections.boxes:
                ...
                label = self.model.names[cls]  # Obtener el nombre de la clase desde el índice
            ...
            rospy.logerr(f"Error en la detección de objetos: {e}")
    ...
    def euler_from_quaternion(self, quat):
        ...
        Args:
            quat (list): Lista de 4 elementos [x, y, z, w].
